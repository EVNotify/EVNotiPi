        ...
        try:
            with self.serial_lock:
                while self.serial.in_waiting:   # Clear the input buffer
                    self.log.warning("Stray data in buffer: %s",
                                     str(self.serial.read(self.serial.in_waiting)))
                    sleep(0.1)

                if self.log.isEnabledFor(logging.DEBUG):
                    self.log.debug("Send command: %s", cmd)
        ...
        try:
            ...
            for line in raw:
                ...
                if frame_type == 0:     # Single frame
                    ...
                    lines = math.ceil((int.from_bytes(
                        bytes.fromhex(str(b'0' + line[4:7], 'ascii')),
    ...
    def sendCommandEx(self, cmd, cantx, canrx):
        ...
        try:
            ...
            for line in raw:
                ...
                if frame_type == 0:     # Single frame
                    if self.log.isEnabledFor(logging.DEBUG):
                        self.log.debug("%s single frame", line)
                    ...
                    if self.log.isEnabledFor(logging.DEBUG):
                        self.log.debug("%s first frame", line)
